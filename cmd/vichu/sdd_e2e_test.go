...
func TestSDDDriveViaCLI(t *testing.T) {
	...
	cli(cmdStage, "close", "--run", rid, "--stage", "implement", "--op-id", "si")

	cli(cmdWorker, "start", "--run", rid, "--stage", "review", "--role", "reviewer", "--op-id", "wr")
	wid := activeWorker(t, store)
	mustWrite(t, filepath.Join(host, "verdict.json"), `{"status":"approved","summary":"ok"}`)
	cli(cmdReview, "complete", "--run", rid, "--worker", wid, "--verdict", filepath.Join(host, "verdict.json"), "--op-id", "rc")

	cli(cmdStage, "close", "--run", rid, "--stage", "verify", "--op-id", "sv")
